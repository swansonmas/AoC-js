...
function stepArrayWeird(arr, index) {
	var step = arr[index];
	if (arr[index] >= 3) {
		arr[index] = arr[index] - 1;
	} else {
...
function func1(strs) {
	strs = strs.map( (line) => parseInt(line, 10));
	var idx = 0;
	var stps = 0;
	while (idx > -1 && idx < strs.length)
	{
		idx = stepArray(strs, idx);
		stps++;
	}
	return stps;
}
