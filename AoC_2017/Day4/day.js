...
var permuteStr = function(input) {
	var fn = function(input) {
		var i, ch;
		for (i = 0; i < input.length; i++) {
			ch = input.slice(i, 1)[0];
			usedChars.push(ch);
			if (input.length == 0) {
				permArr.push(usedChars.slice());
			}
...
function func1(strs) {
	...
	strs.map( line => {
		...
		var pass = [];
		line.map( word => {
			if (pass.includes(word))
			{
				pass.push(undefined);
			} else {
			pass.push(word); }
		});
		if (!pass.includes(undefined)) { count++}
	});
...
function func2(strs) {
	...
	strs.map( line => {
		...
		line.map( word => {
			...
			wordArr = wordArr.toString();
			if (pass.includes(wordArr))
			{
				pass.push(undefined);
			} else {
			pass.push(wordArr); }
