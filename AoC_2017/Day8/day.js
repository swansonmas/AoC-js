...
function execInst(inst, i, regs) {
	switch(inst[1]) {
		case 'cpy':
			...
			break;
		case 'inc':
			regs[inst[0]] += parseInt(inst[2],10);
			break;
		case 'dec':
			regs[inst[0]] -= parseInt(inst[2],10);
			break;
...
function runInst(inst, i, regs) {
	...
	switch(inst[5]) {
		case '>':
			if (regs[inst[4]] > parseInt(inst[6],10)) {
				execInst(inst, i, regs);
			}
			break;
		case '<':
			if (regs[inst[4]] < parseInt(inst[6],10)) {
...
function func2(strs) {
	...
	while (idx < strs.length) {
		idx = runInst(strs[idx], idx, regs);
		if (Object.values(regs).max() > mxreg) {
			mxreg = Object.values(regs).max();
		}
	}
...
var input1 = `b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`;
var input =`d dec 683 if qn == 0
d dec -220 if h == 0
