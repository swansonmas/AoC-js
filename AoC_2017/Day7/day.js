...
//	0:23:31
function func1(strs) {
	strs = strs.map( (line) => line.replace(/\((\d+)\)/,'v $1').split(" "));//.match(/\w+|\d+/g).map( wrd => wrd));//parseInt(line, 10));//
	console.log(strs);
	
	var pgms = buildJSON(strs,0,[1],[2]);
	strs.map( pgm => {
		var re;
		if ((re = pgm.join("").match(/->(.+)/)) != null) {
			var chds = re[1].split(",");
			pgms[pgm[0]]['chds'] = chds;
			chds.map( chd => {
				pgms[chd]['prnt'] = pgm[0];
			});
		} else {
			pgms[pgm[0]]['chds'] = null;
		}
	});
	
	
	
	return strs.filter( pgm => pgms[pgm[0]]['prnt'] == null )[0][0];
}

function weight(progs, rt) {
	if (rt['chds'] == null) {
		return rt['v'];
	} else {
		var cw = rt['chds'].reduce( (tw,chd => tw += weight(progs, chd)), 0);
	}
}

var unb = null;
function balance(progs, rt) {
	
	if (rt['chds'] == null) {
		return parseInt(rt['v'], 10);
	} else {
		var cw = rt['chds'].map( chd => balance(progs, progs[chd]));
		var cnts = cw.counts();
		if (new Set(cw).size != 1 && unb == null) {
			rt['chds'].map( (chd,i) => {
				if (cnts[cw[i]] == 1) {
					unb = parseInt(progs[chd]['v'],10) - (cw[i] - cw[(i+1)%cw.length]);
				}
			});
		}
		return cw.reduce( (t,w) => t += w,0) + parseInt(rt['v'],10);
	}
}
...
function func2(strs) {
	...
	balance(pgms, pgms[strs.filter( pgm => pgms[pgm[0]]['prnt'] == null )[0][0]]);
