...
function func1(strs) {
	var comps = strs[0].slice(7);
	
	var sues = buildJSON(strs, 1, [2, 4, 6], [3, 5, 7]);
	
	var found;
	for (var sue in sues) {
		if (sues.hasOwnProperty(sue))
		{
			var props = 0;
			for (var prop in sues[sue]) {
				if (sues[sue].hasOwnProperty(prop)) {
					if (sues[sue][prop] == trueSue[prop]) {
						props++;
					}
					else
					{
		...
		if (sues.hasOwnProperty(sue))
		{
			...
			for (var prop in sues[sue]) {
				if (sues[sue].hasOwnProperty(prop)) {
					...
					}else if (sues[sue][prop] == trueSue[prop]) {
						props++;
					}
					else
					{
