...
function realRoom(rm) {
	var nme = rm.slice(0,rm.length-2).join("");
	nme = nme.split("").sort().join("");
	var sID = parseInt(rm[rm.length-2],10);
	var cksm = rm[rm.length-1].split("");
	
	var lCnts = new Map();
	var mnc = 0;
	var unm = new Set(nme);
	unm.forEach( ul => {
		var r = new RegExp(ul, 'g');
		var ull = (nme.match(r)||[]).length;
		
		if (cksm.indexOf(ul) == -1 && ull > mnc) {
			mnc = ull;
		}
		lCnts.set(ul,ull);
	});
	...
	var cnt = Infinity,
		idx = 0;
	for (var ck of cksm) {
		//var cntMtc = new RegExp(ck,'g');
		var nCnt = (lCnts.get(ck)||[].length);
		
		if (nCnt < cnt && nCnt >= mnc) {
			idx = ck;
			cnt = nCnt; 
		}
		else if (nCnt == cnt && alpha.indexOf(ck) > alpha.indexOf(idx)) {
			idx = cksm;
		}
		else {
			return 0; 
...
function decryptRoom(rm) {
	var sID = parseInt(rm[rm.length-2],10);
	var nmes = rm.slice(0,rm.length-2);
	var nme = nmes.join(" ");
	var unm = new Set(nme);
	
	var nn = nmes.map(nme => {
		return nme.split("").map( ltr => alpha[(alpha.indexOf(ltr)+sID)%(alpha.length)] ).join("");
...
function func1(strs) {
	...
	strs.map( (line, rm) => { 
		rms[rm] = [];
		line.match(/\w+|\d+/g).map( str => rms[rm].push(str)); 
	});//line.split(" "));
	
	var rR = rms.reduce( (cnt, rm) => cnt + realRoom(rm), 0);
...
function func2(strs) {
	...
	var rN = rms.map( rm => decryptRoom(rm) );
	rN = rN.filter( n => /north/.test(n) );
	return rN;
}
...
var input2 = `qzmt-zixmtkozy-ivhz-343[abcdd]`;
