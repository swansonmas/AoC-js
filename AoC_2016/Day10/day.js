...
function findBot(bts, min, max) {
	var btsReady = Object.keys(bts).filter( btk => bts[btk]['in'].size == 2 );
	for (let btk of btsReady) {
		var vals = Array.from(bts[btk]['in'].values());
		if (vals.min() == min && vals.max() == max) {
			return btk;
		}
	}
	return null;
...
function runBots(bts) {
	var obk = Object.keys(bts);
	while (findBot(bts, lowChip, highChip) == null) {
		var btsFl = Object.keys(bts).filter( btk => bts[btk]['in'].size == 2 );
		var btsReady = Object.keys(bts).filter( btk => bts[btk]['in'].size == 2 );
		btsReady.map( btk => {
			var vals = Array.from(bts[btk]['in'].values());
			if (bts[bts[btk]['high']] != null) {
				bts[bts[btk]['high']]['in'].add(vals.max());
			}
			bts[btk]['in'].delete(vals.max());
			if (bts[bts[btk]['low']] != null) {
				bts[bts[btk]['low']]['in'].add(vals.min());
			}
			bts[btk]['in'].delete(vals.max());
		});
	}
...
function runBots2(bts, outCount) {
	var outs = {};
	var obk = Object.keys(bts);
	while (Object.keys(outs).length < outCount) {
		var btsFl = Object.keys(bts).filter( btk => bts[btk]['in'].size == 2 );
		var btsReady = Object.keys(bts).filter( btk => bts[btk]['in'].size == 2 );
		btsReady.map( btk => {
			var vals = Array.from(bts[btk]['in'].values());
			if (bts[bts[btk]['high']] != null) {
				...
				outs[bts[btk]['high']] = vals.max();
			}
			bts[btk]['in'].delete(vals.max());
			if (bts[bts[btk]['low']] != null) {
				bts[bts[btk]['low']]['in'].add(vals.min());
			} else {
				outs[bts[btk]['low']] = vals.min();
			}
			bts[btk]['in'].delete(vals.max());
		});
	}
...
function func1(strs) {
	...
	var valstr = strs.filter( line => line.startsWith('value')).map( val => val.replace(/bot\s(\d+)/g,'bot$1').split(" "));
	var btstr = strs.filter( line => line.startsWith('bot')).map( bt => bt.replace(/\s(\d+)/g,'$1').split(" "));
	
	var bots = buildJSON(btstr, 0, [2, 6], [4,8]);
	btstr.map( b => bots[b[0]]['in'] = new Set() );
	
	valstr.map( val => bots[val[4]]['in'].add(parseInt(val[1],10)));
	
	runBots(bots);
	
	return findBot(bots, lowChip, highChip).replace("bot","");
}
...
function func2(strs) {
	...
	var valstr = strs.filter( line => line.startsWith('value')).map( val => val.replace(/bot\s(\d+)/g,'bot$1').split(" "));
	var btstr = strs.filter( line => line.startsWith('bot')).map( bt => bt.replace(/\s(\d+)/g,'$1').split(" "));
	var outs = strs.filter( line => /output/.test(line)).map( ot => ot.match(/output \d+/g).length).reduce( (os,ot) => os += ot, 0);
	...
	var outs = runBots2(bots, outs);
	
	return outputs.reduce( (val,op) => val *= outs['output'+op],1);
}
var outputs = [0, 1, 2];
var lowChip = 17;//2;
var highChip = 61;//5;
...
var input = `bot 152 gives low to bot 155 and high to bot 70
