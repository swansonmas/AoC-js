...
function genNodes(list) {
	var nds = {};
	list.map( l => {
		var re = /.+x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T\s+.+/.exec(l);
		var [_, X, Y, Size, Used, Avail] = re.map( r => Number(r) );
		nds[[parseInt(re[1],10),parseInt(re[2],10)]] = {'XY': [parseInt(re[1],10),parseInt(re[2],10)], 'Size': parseInt(re[3],10), 'Used': parseInt(re[4],10), 'Avail': parseInt(re[5],10)};
	});
	
	return nds;
}

function genGrid(nodes, opens) {
	var grd = Array(31).fill(0);
	for (var i = 0; i < 30; i++) {
		grd[i] = Array(31).fill(0);
	}
	opens.map( pr => {
		grd[nodes[pr[0]]['XY'][1]][nodes[pr[0]]['XY'][0]] = 1;
		grd[nodes[pr[1]]['XY'][1]][nodes[pr[1]]['XY'][0]] = 1;
	});
	return grd;
}
...
function func1(strs) {
	...
	var nds = genNodes(strs.slice(2));
	var kys = Object.keys(nds);
	var prm = combine(kys, 2, 2);
	var cnt = prm.filter( pr => isViable(nds[pr[0]],nds[pr[1]]) || isViable(nds[pr[1]],nds[pr[0]]) );
...
function func2(strs) {
	...
	var prm = combine(kys, 2, 2);
	var cnt = prm.filter( pr => isViable(nds[pr[0]],nds[pr[1]]) || isViable(nds[pr[1]],nds[pr[0]]) );
	var grd = genGrid(nds, cnt);
	grd = new Graph(grd);
	var start = grd.grid[27][13];
	var end = grd.grid[0][29];
	var srch = astar.search(grd,start,end);
	var len = srch.length + 1 + (5*(29));
