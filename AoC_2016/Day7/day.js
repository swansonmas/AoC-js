...
function func1(strs) {
	...
	var ip = 0;
	var abba = new RegExp(/(\w)((?!\1)\w)\2\1/);
	inhs.map( (inh,i) => {
		if (abba.test(ouths[i]) && !abba.test(inh)) {
			ip++;
		}
	});
	var tst = strs[0].match(/(.+)\[.+\](.+)/);
	//console.log(strs);
...
function func2(strs) {
	...
	var ip = [];
	var aba = new RegExp(/(?=(\w)((?!\1)\w)\1)./g);
	var ins = [];
	inhs.map( (inh,i) => {
		var regl;
		var regs = '';
		while((regl = aba.exec(inh)) !== null) {
			regs += regl[0] + ' ';
			var bab = new RegExp(regl[2] + regl[1] + regl[2]);
			if (bab.test(ouths[i])) {
				ip.push(i);
			}
		}
